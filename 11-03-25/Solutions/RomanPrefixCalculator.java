...
public class RomanPrefixCalculator {
	...
	private static int RomanNumeralToInt(String numeral) {
		List<Character> numeralChr = Arrays.asList('I', 'V', 'X', 'L', 'C');
		int numeralVals[] = {0, 1, 5, 10, 50, 100};

		int sum = 0;
		int buffer = 0;
		for (int i = 0; i < numeral.length(); i++) {
			int chrVal = numeralVals[numeralChr.indexOf(numeral.charAt(i))+1];
			
			if (chrVal > buffer && i > 0) {
				sum += (chrVal - buffer);
				buffer = 0;
				continue;
			}
	...
	public static int EvaluateRomanPrefixNotation(String str) {
		...
		for (int i = 0; i < split.length; i++) {
			if (split[i].charAt(0) == '+' || split[i].charAt(0) == '-' || split[i].charAt(0) == '*' || split[i].charAt(0) == '/')
				operations.push(split[i].charAt(0));
			else
				numbers.push(RomanNumeralToInt(split[i]));
		...
		return Math.abs(numbers.firstElement());
	}
