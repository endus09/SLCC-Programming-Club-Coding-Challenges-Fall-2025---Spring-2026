...
class Rule110 {
	...
	private static int getIndex(String[] arr, String find) {
		for (int i = 0; i < arr.length; i++)
			if (arr[i].equals(find)) return i;
		return -1;
	}
	private static char boundsCheck(char[] board, int index) {
		if (index < 0 || index >= board.length) return '0';
		return board[index];
	}
	...
	public static char[] stepBoard(char[] lastBoard, String[] neighborRule, char[] newCellRule) {
		char[] newBoard = new char[lastBoard.length];
		for (int i = 0; i < lastBoard.length; i++) {
			StringBuilder sb = new StringBuilder();
			sb.append(boundsCheck(lastBoard, i-1));
			sb.append(boundsCheck(lastBoard, i));
			sb.append(boundsCheck(lastBoard, i+1));
			int neighborIndex = getIndex(neighborRule, sb.toString());

			newBoard[i] = newCellRule[neighborIndex];
		}
		return newBoard;
	}
